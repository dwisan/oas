            ...
            if (preg_match('/(resend|verified)/', $action, $match)) {
                ...
                if ($user) {
                    if ($match[1] === 'resend') {
                        ...
                        // save OTP
                        $db->update($table, $user->id, array(
                            'activatecode' => $otp.':'.$expired
                        ));
                        // send SMS
                        $msg = Language::replace('Your OTP code is :otp. Please enter this code on the website to confirm your phone number.', array(':otp' => $otp));
                        // send SMS
