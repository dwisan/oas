...
class View extends \Kotchasan\View
{
    ...
    public static function usernameLabel($separator = '/')
    {
        ...
        foreach (self::$cfg->login_fields as $item) {
            $result[] = '{LNG_'.ucfirst($item).'}';
        }
        return implode($separator, $result);
    }

    /**
     * คืนค่าเวลาตงเหลือสำหรับการขอ OTP
     *
     * @param int $otp_timeout
     *
     * @return string
     */
    public static function getTimeElapsed($otp_timeout)
    {
        $time = time();
        if ($time > $otp_timeout) {
            return '00:00';
        }
        $diff = $otp_timeout - $time;
        return sprintf('%02d', floor($diff / 60)).':'.sprintf('%02d', floor($diff % 60));
    }
}
