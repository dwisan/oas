        ...
        if (!empty($params['username']) && (!empty($params['password']) || !empty($params['token']))) {
            ...
            if (is_array($login_result)) {
                ...
                if (is_string($login_result)) {
                    if (preg_match('/^([0-9]{4,4}):([0-9]+)$/', $login_result, $codes)) {
                        // ยังไม่ได้ verfied Phone number
    ...
    public static function checkMember($params)
    {
        ...
        if ($login_result === null && !empty($params['username'])) {
            ...
            if (isset($item)) {
                ...
                } elseif (preg_match('/^[0-9]{10,10}$/', $item['username']) && preg_match('/^([0-9]{4,4}):([0-9]+)$/', $item['activatecode'], $codes)) {
                    // ยังไม่ได้ verfied Phone number
                    return $item['username'].':'.$codes[2];
                } else {
